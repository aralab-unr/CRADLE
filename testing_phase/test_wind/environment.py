        ...
        for i, j in fire_neighbor:
            ...
            for k, l in zip(x_nei_coords.flatten(), y_nei_coords.flatten()):
                if (k,l) in self.fire_set:
                    ...
                    if d_nmkl < self.dmax:
                        P_nmkl_0 = (self.K * (1/(d_nmkl**2)))
                        d_nmkl_angle = np.degrees(np.arctan2(j - l, i - k))
                        vec_diff = d_nmkl_angle - self.wind[0]
                        if vec_diff < 0:
                            vec_diff += 360
                        P_nmkl_w = ((self.K*(self.wind[1]*d_nmkl*np.cos(vec_diff*np.pi/180)))/(d_nmkl**2))
                        # print(self.wind, P_nmkl_0, P_nmkl_w)
                        Pnmkl *= (1 - max(0, min(1, (P_nmkl_0 + P_nmkl_w))))
