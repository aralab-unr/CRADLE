...
class FireEnvironment(gym.Env):
    ...
    def reset(self):
        self.binary_val = np.full([self.x_len, self.y_len], 0)
    ...
    def step(self, obss, action):
        ...
        for idx, obs in enumerate(obss):
            ...
            if action[idx] == "North":
                obs.vector.pos.x -= 1
            elif action[idx] == "South":
                obs.vector.pos.x += 1
            elif action[idx] == "East":
                obs.vector.pos.y += 1
            elif action[idx] == "West":
                obs.vector.pos.y -= 1
            ...
            rew_list.append(self.reward(obs))
            new_obss.append(obs)

        return np.array(new_obss), np.array(rew_list), False, ""
    ...
    def reward(self, obs):
        ...
        obs.image.coverage_map[x_view_min:x_view_max, y_view_min:y_view_max] = np.full(((x_view_max-x_view_min), (y_view_max-y_view_min)), 255)
        ...
        # # Stores the frames of fire and fuel over entire duration.
        # fire_frames = []
        # fuel_frames = []

        # fire_frames.append(binary_val)
        # fuel_frames.append(fuel_map)

        # # Reading natural parameters
        # for _ in range(320):
        #   fire_neighbor = set()
        #   fire_off = set()
        #   for i, j in fire_set:
        #     x_nei = np.arange(i-2,i+2+1)
        #     x_nei = x_nei[(x_nei>=0) & (x_nei<100)]
        #     y_nei = np.arange(j-2, j+2+1)
        #     y_nei = y_nei[(y_nei>=0) & (y_nei<100)]
        #     x_nei_coords, y_nei_coords = np.meshgrid(x_nei, y_nei)
        #     for k, l in zip(x_nei_coords.flatten(), y_nei_coords.flatten()):
        #       if (k,l) not in fire_set:
        #         if fuel_map[i][j] > 0:
        #           fire_neighbor.add((k,l))
        #     if fuel_map[i][j] > 0:
        #       fuel_map[i][j] -= 1
        ...
        #         d_nmkl = np.linalg.norm(np.array([i,j]) - np.array([k,l]))
        #         if d_nmkl < dmax:
        #           Pnmkl *= (1 - max(0, min(1, ((K * (1/(d_nmkl**2)))))))
        #     unif_comp = np.random.uniform(0,1,1)
        #     # Stochastically ignite fire
        #     if ((1 - Pnmkl)>=unif_comp)[0]:
        #       new_fire_set.add((i,j))
        ...
        #     #total_count+=1
        #   for new_fire in new_fire_set:
        #     fire_set.add(new_fire)
        #     binary_val[new_fire[0]][new_fire[1]] = 1
        #   #if total_count==0 : total_count = 1
        #   #print(np.round((burn_count/total_count)*100),"% (",burn_count," / ",total_count,")")
            
        #   for ind_fire_off in fire_off:
        #     fire_set.remove(ind_fire_off)
        #     binary_val[ind_fire_off[0]][ind_fire_off[1]] = 0
        
        #   fire_frames.append(binary_val.copy())
        #   fuel_frames.append(fuel_map.copy())
