...
class DDQN_Agent(): 
    ...
    def __init__(self, x_len, y_len, n_vector_obs, n_image_obs, n_image_channel, n_actions, batch_size, memory_size, 
                 update_step, learning_rate, gamma, tau, model_path, id):
        ...
        self.localVDN = LocalizedVDN( w = 0.5)
    ...
    def check_train(self):
        ...
        if self.steps == 0 and self.get_mem_cntr() >= self.batch_size:
            return True
        else:
            return False
    ...
    def learn(self, batch_idx, Nei_Q, soft_copy=True):
        ...
        # dones: batch_size, 1
        vec_states, img_states, actions, rewards, next_vec_states, next_img_states, dones = self.memory.sampleByIndex(batch_idx)
        ...
        dones = torch.tensor(dones).reshape(-1, 1).float().to(self.device)
