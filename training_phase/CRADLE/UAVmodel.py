...
class UAV:
    def __init__ (self, x_len, y_len, n_drones, n_vector_obs, n_image_obs, n_image_channel, n_actions, 
        ...
        self.agent = DDQN_Agent(x_len, y_len, n_vector_obs, n_image_obs = n_image_obs, n_image_channel = n_image_channel, 
                                ...
                                tau = tau, model_path = model_path, id = id)
        self.neighbors = []
    ...
    def search_neighbors(self, other_uavs_shared_pos):
        nei = []
        curr_pos = self.get_position()
        for other_uav in other_uavs_shared_pos:
            other_uav_pos = other_uav["agent_position"]
            if np.sqrt( (other_uav_pos[0] - curr_pos[0])**2 + (other_uav_pos[1] - curr_pos[1])**2 ) <= self.desired_comm_dist:
                nei.append(other_uav["id"])
        self.neighbors = nei
    ...
    def share_position(self):
        return { "id" : self.id, "agent_position" : self.obs.share_position()}
    
    def share_obs(self):
        return { "id" : self.id, "agent_observation" : self.obs.share_obs()}
    
    def UpdateObsFromNeighbor(self, obs):
        nei_id = obs["id"]
        nei_obs = obs["agent_observation"]
    ...
    def act(self, vector_obs, mod_map, epsilon):
        return self.agent.act(vector_obs, mod_map, epsilon)
    
    def step(self, cur_vec_state, cur_img_state, action, reward, next_vec_state, next_img_state, done):
        self.agent.step(cur_vec_state, cur_img_state, action, reward, next_vec_state, next_img_state, done)

    def check_train(self):
        return self.agent.check_train()
    ...
    def learn(self, batch_idx, list_of_Nei_Q):
        ...
        self.agent.learn(batch_idx, (Nei_policy_Q_list, Nei_target_Q_list))
