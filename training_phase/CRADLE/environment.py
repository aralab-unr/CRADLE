...
class FireEnvironment(gym.Env):
    ...
    def step(self, uav, action):
        ...
        direct_rew = 0
        if ((uav.obs.vector.pos.x < 0) or (uav.obs.vector.pos.x >= self.x_len)):
            direct_rew = -5
        if ((uav.obs.vector.pos.y < 0) or (uav.obs.vector.pos.y >= self.y_len)):
            direct_rew = -5
    ...
    def reward(self, uav):
        ...
        # affectd_rew = reward
        for Nei_obs in uav.neighbors.values():
            ...
            if dist_k <= self.r_collision_avoidance:
                reward = -1
        #         affectd_rew_temp = ((reward / 2) - ((reward + 2) * (self.r_collision_avoidance - dist_k) / (2 * self.r_collision_avoidance)))
        #         if affectd_rew_temp < affectd_rew:
        #             affectd_rew = affectd_rew_temp
        # reward = affectd_rew
