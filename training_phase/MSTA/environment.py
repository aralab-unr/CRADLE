...
class FireEnvironment(gym.Env):
    ...
    def __init__ (self, x_len, y_len, r_collision_avoidance):
        super().__init__()
    ...
    def step(self, obs, obss, action):

        obs.image.coverage_map[obs.image.coverage_map > 0] -= 1
        ...
        # Negative rewards for UAV crossing the boundary lines.
        direct_rew = 0
        if ((obs.vector.pos.x < 0) or (obs.vector.pos.x >= self.x_len)):
            direct_rew = -5
        if ((obs.vector.pos.y < 0) or (obs.vector.pos.y >= self.y_len)):
            direct_rew = -5

        obs.vector.pos.x = self.correct_coords(obs.vector.pos.x, self.x_len)
        obs.vector.pos.y = self.correct_coords(obs.vector.pos.y, self.y_len)

        rew = self.reward(obs, obss)
    ...
    def render(self, obss):
        xs = []
        ys = []
        for obs in obss:
            xs.append(int(obs.vector.pos.x))
            ys.append(int(obs.vector.pos.y))
    ...
    def print_env(self, xs, ys):
        ...
        cv.rectangle(img, (0,0), (100, 100), (34,139,34), thickness = -1)
        ...
        display = cv.resize(img, (500, 500)) 
        cv.imshow("filled", display)
        cv.waitKey(1)

    def reward(self, obs, obss):
        x_view = int(obs.vector.pos.x)
        y_view = int(obs.vector.pos.y)
        ...
        reward = np.sum((self.binary_val[x_view_min:x_view_max, y_view_min:y_view_max] - obs.image.belief_map[x_view_min:x_view_max, y_view_min:y_view_max]).clip(min = 0))

        # print(obs.id ,reward, "[", np.sum(self.binary_val[x_view_min:x_view_max, y_view_min:y_view_max]), "/", np.sum(self.binary_val), "] [", np.sum(obs.image.belief_map[x_view_min:x_view_max, y_view_min:y_view_max]), "/", np.sum(obs.image.belief_map), "]", np.sum((self.binary_val[x_view_min:x_view_max, y_view_min:y_view_max] - obs.image.belief_map[x_view_min:x_view_max, y_view_min:y_view_max]) < 0), len(self.fire_off))

        obs.image.belief_map[x_view_min:x_view_max, y_view_min:y_view_max] = self.binary_val[x_view_min:x_view_max, y_view_min:y_view_max]
        obs.image.coverage_map[x_view_min:x_view_max, y_view_min:y_view_max] = np.full(((x_view_max-x_view_min), (y_view_max-y_view_min)), 255, dtype=np.uint8)

        # print(np.sum(obs.image.belief_map[x_view_min:x_view_max, y_view_min:y_view_max]), "/", np.sum(obs.image.belief_map))

        for other_obs in obss:
            nei_x = int(other_obs.vector.pos.x)
            nei_y = int(other_obs.vector.pos.y)
            if obs.id != other_obs.id:
                dist_k = np.sqrt( (nei_x - x_view)**2 + (nei_y - y_view)**2 )
                if dist_k <= self.r_collision_avoidance:
                    reward = -1
